...
export function createPlugin(patterns, lang) {
  ...
  const source = 'retext-anti-woke' + (lang === 'en' ? '' : '-' + lang)
  ...
  /**
   * Check potentially woke language.
   *
   ...
   *   Nothing.
   */
  /* This newer way of formatting the warn messages breaks compatibility with
  Chad. We comment this out and implement something similar to the older way
  of doing this until they fix this upstream.
  function warn(file, match, pattern) {
    ...
    const message = file.message(
      'Unexpected potentially woke use of ' +
        quotation(actual, '`') +
    ...
    message.url = 'https://github.com/nerditron/retext-anti-woke#readme'
  } */
  /**
   * @param {VFile} file
   *   File.
  ...
  function warn(file, match, pattern) {
    ...
    const start = pointStart(match.nodes[0]) // Start of the first word
    const end = pointEnd(match.nodes[match.nodes.length - 1]) // End of the last word
    ...
    // Revert to 6.7.0 style: attach message to the first node
    const message = file.message(
      'Unexpected potentially woke use of ' +
        quotation(actual, '`') +
        (pattern.condition ? ', ' + pattern.condition : '') +
        ...
        (expected
          ? 'in some cases ' +
            ...
            ' may be better'
          : 'try not to use it'),
      // Match.nodes[0], // Use the first node for position
      end && start ? {start, end} : undefined,
      [source, pattern.id].join(':') // Use source:ruleId format for ruleId
    )

    message.actual = actual
    message.expected = expected
    message.note = pattern.note
    message.url = 'https://github.com/nerditron/retext-anti-woke'
  }
}
